...
def put_data_to_bins_sagittal():
    ...
    # Iterate over the rows and count them
        ...
        for row in reader:
            ...
            if sagittal_bins[0] < sum_voxel <= sagittal_bins[1]:
                ...
                data_bins_sagittal['data_bin_1'].append(data_bins)



            elif sagittal_bins[1] < sum_voxel <= sagittal_bins[2]:
                ...
                data_bins_sagittal['data_bin_2'].append(data_bins)


            elif sagittal_bins[2] < sum_voxel <= sagittal_bins[3]:
                ...
                data_bins_sagittal['data_bin_3'].append(data_bins)


            elif sagittal_bins[3]< sum_voxel <=sagittal_bins[4]:
                ...
                data_bins_sagittal['data_bin_4'].append(data_bins)

            
            elif sagittal_bins[3] < sum_voxel <= sagittal_bins[5]:
                ...
                data_bins_sagittal['data_bin_5'].append(data_bins)
...
number_of_sagittal_slice,data_bins_sagittal=put_data_to_bins_sagittal()
...
def find_empty_fold():
    ...
    for fold in range(1,number_of_fold+1):
        ...
        if len(sagittal_folds[f'axial_fold_{fold}']) < size_each_fold:
            empty_fold=fold
            break
...
print(bin_allowness_for_each_fold)
...
while( sum_fold < size_each_fold * 5 ): 
    ...
    for bins_intialization in range(1,len_sagittal_bin+1):


        len_bin_n=len(data_bins_sagittal[f'data_bin_{bins_intialization}'])
        ...
        while(len_bin_n > 0): 
            ...
            len_bin_n_start=len(data_bins_sagittal[f'data_bin_{bins_intialization}'])



            initial_random = random.choice(data_bins_sagittal[f'data_bin_{bins_intialization}'])
            ...
            for bins_searching in range(1,len_sagittal_bin+1):

                d=data_bins_sagittal[f'data_bin_{bins_searching}']
                ...
                for entry in desired_entries:
                    ...
                    if (bin_picked_for_each_fold[f'data_bin_{bins_searching}'][f'fold_{empty_fold}'] ) < bin_allowness_for_each_fold[f'data_bin_{bins_searching}']:
                        ...
                        sagittal_folds[f'axial_fold_{empty_fold}'].append(item)


                        data_bins_sagittal[f'data_bin_{bins_searching}'].remove(item)
...
# Sagittal bins
# bins = [0,1000,2000,3000,5000,7000,float('inf')]
